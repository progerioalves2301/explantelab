...
void abrirPortalWifi(bool forcar) {
  ...
  // v2.6.13 — não existe código de balança: a credencial da balança associada
  // é buscada automaticamente com o token da prateleira.
...
// v2.6.13 — busca a credencial da balança associada usando o token da prateleira.
bool buscarCredencialBalanca() {
  if (WiFi.status() != WL_CONNECTED || creds.device_token.length() < 8) return false;
  HTTPClient h;
  WiFiClientSecure c;
  c.setInsecure();
  String url = String(API_HOST) + "/api/public/scale/claim";
  if (!h.begin(c, url)) return false;
  h.setTimeout(8000);
  h.addHeader("X-Device-Token", creds.device_token);
  int code = h.GET();
  String resp = h.getString();
  h.end();
  if (code == 404) { Serial.println("[SCALE] nenhuma balança associada a esta prateleira"); return false; }
  if (code < 200 || code >= 300) {
    Serial.printf("[SCALE] claim HTTP %d: %s\n", code, resp.c_str());
    return false;
  }
  JsonDocument doc;
  ...
  g_token_scale = token;
  salvarPerifericos();
  Serial.println("[SCALE] credencial da balança obtida automaticamente");
  return true;
}
...
void tickBalanca(unsigned long now) {
  ...
  if (g_hx_pode_amostrar && (g_ts_ultimo_hx_envio == 0 || now - g_ts_ultimo_hx_envio >= 10000UL)) {
