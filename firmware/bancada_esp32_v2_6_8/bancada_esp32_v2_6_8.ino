...
static const char* FIRMWARE_VERSION = "2.6.8";
...
// -------- CO2 (SCD41) --------
// v2.6.7: varredura I2C, wake-up/reinit e retentativa periódica.
void scanI2c() {
  ...
  if (achados == 0)
    Serial.println("[I2C] NENHUM dispositivo encontrado — verifique VCC(3V3), GND, SDA=21, SCL=22 e pull-ups");
}

void iniciarScd41() {
  // Wire.begin() já foi chamado pelo bloco do DS3231; reaproveitamos o bus.
  Wire.setClock(50000);          // SCD41 é lento; 50 kHz é mais tolerante a cabos longos
  scanI2c();

  // Presença física no endereço 0x62 antes de tentar comandos.
  Wire.beginTransmission(0x62);
  bool presente = (Wire.endTransmission() == 0);
  if (!presente) {
    Serial.println("[SCD41] não respondeu em 0x62 — CO2 desabilitado (tentarei de novo em 60 s)");
    g_tem_scd41 = false;
    return;
  }

  g_scd4x.begin(Wire, SCD41_I2C_ADDR_62);
  g_scd4x.wakeUp();                      // pode estar em power-down
  delay(30);
  g_scd4x.stopPeriodicMeasurement();     // ignora erro se já estava parado
  delay(600);
  g_scd4x.reinit();
  delay(30);
  ...
  if (errSn == 0)
    ...
    Serial.printf("[SCD41] getSerialNumber err=%u\n", errSn);
  ...
  if (err) {
    ...
    Serial.println("[SCD41] modo periódico OK (1 amostra a cada 5 s; 1a leitura em ~5-10 s)");
    g_tem_scd41 = true;
  }
  Serial.printf("[CO2] token configurado: %s\n",
                g_token_co2.length() >= 8 ? "SIM" : "NAO (preencha no portal Wi-Fi)");
}

void tickCo2(unsigned long now) {
  // v2.6.7: se não detectado, retenta a cada 60 s (sensor pode ligar depois).
  if (!g_tem_scd41) {
    static unsigned long ts_retry = 0;
    if (now - ts_retry >= 60000UL) {
      ts_retry = now;
  ...
  if (now - g_ts_ultima_co2_leitura >= 5000UL) {
    ...
    g_co2_soma    += ppm;
    g_co2_amostras++;
    Serial.printf("[CO2] %u ppm | %.1f C | %.0f %%UR (amostras=%u)\n",
                  (unsigned)ppm, t, rh, (unsigned)g_co2_amostras);
  }
  ...
  if (now - g_ts_ultimo_co2_envio >= 60000UL && g_co2_amostras > 0) {
    ...
    if (g_token_co2.length() < 8) {
      Serial.printf("[CO2] media %u ppm NAO enviada — token do sensor vazio\n",
                    (unsigned)media);
      return;
    }
